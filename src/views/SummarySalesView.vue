...
const summarySales = computed(() => {
  if (!mainStore.apiData.summary_sales.isLoading) {
    try {
      return {
        isLoading: false,
        data: mainStore.apiData.summary_sales.data[0].Transaction,
      };
    } catch {
...
const itemStatistic = computed(() => {
  ...
  const average_net_pax =
    (grand_total - summarySales.value?.data?.discount) / total_guest;
  // console.log(
